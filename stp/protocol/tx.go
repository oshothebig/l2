...
func (p *StpPort) TxRSTP() {
	...
	rstp := layers.RSTP{
		...
		MsgAge:            uint16(p.RootTimes.MessageAge << 8),
		MaxAge:            uint16(p.RootTimes.MaxAge << 8),
		HelloTime:         uint16(p.RootTimes.HelloTime << 8),
		FwdDelay:          uint16(p.RootTimes.ForwardingDelay << 8),
